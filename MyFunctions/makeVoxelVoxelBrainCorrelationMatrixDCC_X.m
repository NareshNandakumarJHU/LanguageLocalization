function wholebrain = makeVoxelVoxelBrainCorrelationMatrixDCC_X(X,Y,Z,img,comparisonseed,CorrelationMatrix)
    ...
    for y = 1:Y
        for z = 1:Z
            ...
            if sum(sum(comparisonVector ~=0)) %error in DCC if one of the two inputs is a zero vector
                [~, Ct] = runDCC_Xtwovector(comparisonseed,comparisonVector);
